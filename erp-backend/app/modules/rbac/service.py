...
class RoleService(BaseCRUDService[Role, RoleCreate, RoleUpdate]):
    ...
    async def get_by_name(self, name: str) -> Optional[Role]:
        return await self.repository.get_by_name(self.db, name)

    async def get_by_code(self, code: str) -> Optional[Role]:
        return await self.repository.get_by_code(self.db, code)
    ...
    async def assign_permissions(self, data: PermissionAssign, user_id: UUID) -> dict:
        ...
        await self.log_audit_event(
            ...
            new_value={
                "role_id": str(role.id),
                "permissions": [str(pid) for pid in data.permission_ids],
            },
        )
    ...
    async def assign_modules(self, role_id: UUID, module_ids: List[UUID], user_id: UUID) -> dict:
        ...
        if module_ids:
            ...
            await self.log_audit_event(
                ...
                "Role",
                user_id,
                new_value={"role_id": str(role_id), "modules": [str(m) for m in module_ids]},
            )
