...
import { Play, Pause, Search, AlertCircle, CheckCircle2, UploadCloud, Upload } from 'lucide-react';
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
            <div className="flex flex-wrap gap-2 w-full md:w-auto">
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2 text-xs font-bold transition-all border rounded-lg ${lmsCategoryFilter === cat.key
            ...
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-4 py-2 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-500"
              />
            </div>
          </div>
              ...
              <div key={course.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col justify-between hover:border-blue-600/40 hover:-translate-y-1 hover:shadow-lg transition-all duration-300 ease-out cursor-pointer">
                ...
                <div className="p-6 space-y-4">
                  <div className="flex justify-between items-start gap-2">
                    ...
                    {course.progress === 100 && (
                      <span className="text-[9px] font-bold text-emerald-600 uppercase tracking-widest bg-emerald-50 border border-emerald-100 px-2 py-0.5 animate-pulse rounded-sm">
                        ✓ Cleared
                      </span>
                    )}
                  </div>
                  ...
                  {course.progress === 100 ? (
                    <button
                      ...
                      className="w-full py-2.5 bg-emerald-600 rounded-lg hover:bg-emerald-700 text-white text-xs font-bold transition-all uppercase tracking-wider"
                    >
                      Claim Certificate
                    </button>
                  ) : (
                    <button
                      ...
                      className="w-full py-2.5 bg-slate-50 border border-slate-200 rounded-lg hover:bg-blue-600 hover:text-white hover:border-blue-600 text-xs font-bold text-slate-700 transition-all uppercase tracking-wider"
                    >
                      Open Curriculum
                    </button>
                  )}
