...
export default function TaskDashboardPage() {
  ...
  return (
    <div className="space-y-6 animate-slide-in select-none">
      ...
      {/* Header */}
      <div>
        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Task Dashboard</h2>
        <p className="text-sm text-slate-550 mt-1">Audit published assignments, track student submission percentages, and monitor overdue milestones.</p>
      </div>

      {/* KPI Cards Row */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total Published Milestones</span>
          <h3 className="text-3xl font-black text-slate-900 mt-1">{totalTasks} Tasks</h3>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Overall Submission Rate</span>
      ...
      <div className="space-y-4">
        <h3 className="font-bold text-xs text-slate-400 uppercase tracking-widest">Milestones Directory</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {tasks.map(task => {
            ...
            return (
              <div 
                key={task.id}
                className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-indigo-400 transition-all flex flex-col justify-between space-y-4"
              >
                <div>
                  ...
                  <h4 className="text-base font-black text-slate-900 mt-3">{task.title}</h4>
                  <p className="text-xs text-slate-500 mt-1 leading-relaxed line-clamp-2">{task.description}</p>
                </div>

                <div className="pt-3 border-t border-slate-50 flex justify-between items-center text-[10px] font-bold text-slate-500">
